...
class Env(gym.Env):
    def __init__(
        ...
        self.low_obs = np.array([self.x_low_obs, self.y_low_obs, self.z_low_obs])
        self.high_obs = np.array([self.x_high_obs, self.y_high_obs, self.z_high_obs])
    ...
    def _reward(self):
        ...
        if self.distance_threshold_min != self.distance_threshold_max:
            reward += 5 / ((self.distance_threshold - self.distance_threshold_min)/(self.distance_threshold_max - self.distance_threshold_min))
